...
No real MySQL server is assumed to be reachable in this environment. Every
DB-touching test in this suite drives `features/auth/service.py` through a
fake PyMySQL-shaped connection/cursor so the auth business logic can be
exercised without any network or server dependency.
"""
...
class FakeCursor:
    ...
    def execute(self, sql, params=None):
        ...
        if self.auto_increment_lastrowid and sql.strip().upper().startswith("INSERT"):
...
class FakeConnection:
    ...
    def close(self):
        # No-op: real PyMySQL connections are closed by every UI page in a
...
@pytest.fixture
def integrity_error():
    return pymysql.err.IntegrityError(1062, "Duplicate entry")
